...
# Find the most frequent winning numbers in 2023 using pandas.Series and lambda function
...
def find_frequencies(series):
    values, counts = np.unique(series, return_counts=True)
    return values[np.argmax(counts)], values[np.argmin(counts)]

# Find the most and least frequent winning numbers in June using pandas.Series and lambda function
